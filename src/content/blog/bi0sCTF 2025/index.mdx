...
`{ "bio": "hello", "&bio": "<h1>helloo hi browser</h1>" }` to /update_bio we should be able to succesfully inject Html that will render when a users bio is viewed . I had to test this locally since only admins can view users bio : 
